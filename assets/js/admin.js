...
(function () {
  ...
  document.addEventListener('keyup', function (e) {
    if (typeof e.target.dataset.fixerApi === 'undefined') return;
  ...
  document.addEventListener('focusout', function (e) {
    ...
    if (e.target.dataset.state === 'dirty' && e.target.value.length > 0) {
      messageEl.textContent = 'Loading...'; // alert('content changed')

      if (window.fetch) {
        fetch(ajaxurl + '?action=wtecc_test&access_key=' + e.target.value).then(function (res) {
          return res.json();
  ...
  document.addEventListener('blur', function (e) {
    if (e.target.id !== 'wptecc-purge') return;
    e.preventDefault();
    fetch && fetch(ajaxurl + '?action=wtecc_purge&_nonce=' + e.target.dataset.nonce).then(function (res) {
